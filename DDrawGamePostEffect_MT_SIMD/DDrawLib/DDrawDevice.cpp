...
#include "../Filter/ImageProcessor.h"
...
BOOL CDDrawDevice::CreateBackBuffer(DWORD dwWidth, DWORD dwHeight)
{
	...
	// post effect
	m_dwPostEffectBufferWidth = m_dwWidth;
	m_dwPostEffectBufferHeight = m_dwHeight;
	m_pPostEffectBuffer = (char*)_aligned_malloc(m_dwWidth * m_dwHeight * 4, 32);
	memset(m_pPostEffectBuffer, 0, m_dwWidth * m_dwHeight * 4);
...
BOOL CDDrawDevice::BeginDraw(BOOL bUsePostEffect, CImageProcessor* pImageProcessor)
{
	BOOL	bResult = FALSE;

	if (bUsePostEffect)
	{
		m_pWriteBuffer = m_pPostEffectBuffer;
		m_dwWriteBufferPitch = m_dwWidth * 4;
		m_pCurImageProcessr = pImageProcessor;
	}
	else
	{
...
void CDDrawDevice::EndDraw()
{
	if (m_bUsePostEffect)
	{
		...
		if (dwBufferHeight != m_dwHeight)
			__debugbreak();
	#endif
		m_pCurImageProcessr->ProcessImage(pBuffer, dwPitch, m_pWriteBuffer, m_dwWidth, m_dwHeight, m_dwWriteBufferPitch);
		UnlockBackBuffer();
		m_bUsePostEffect = FALSE;
		m_pCurImageProcessr = nullptr;
	}
	else
	{
		UnlockBackBuffer();
...
void CDDrawDevice::DrawInfo(HDC hDC)
{
	...
	WriteText(wchTxt, dwLen, 0, 0, 0xff00ff00, hDC);
}
...
{
	if (m_pPostEffectBuffer)
	{
		_aligned_free(m_pPostEffectBuffer);
		m_pPostEffectBuffer = nullptr;
		m_dwPostEffectBufferWidth = 0;
		m_dwPostEffectBufferHeight = 0;
	}
