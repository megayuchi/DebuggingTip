...
BOOL CGame::Initialize(HWND hWnd)
{
	...
	m_ppAmmoList = new CFlightObject*[MAX_AMMO_NUM];
	memset(m_ppAmmoList, 0, sizeof(CFlightObject*) * MAX_AMMO_NUM);
	return TRUE;
}
void CGame::Process()
{
...
void CGame::DrawScene()
{
	...
	m_pDDraw->BeginDraw(m_bUsePostEffect, m_pImageProcessor);
...
void CGame::TogglePostEffect()
{
	m_bUsePostEffect = (m_bUsePostEffect == 0);
}
void CGame::ToggleMutlThread()
{
	BOOL bSwitch = m_pImageProcessor->IsMultiThreadEnabled();
	bSwitch = bSwitch == 0;
	m_pImageProcessor->EnableMultThread(bSwitch);
}
void CGame::ToggleSIMD()
{
	BOOL bSwitch = m_pImageProcessor->IsSIMDEnabled();
	bSwitch = bSwitch == 0;
	m_pImageProcessor->EnableSIMD(bSwitch);
}
